...
export function getAppName() {
  ...
  const iosDir = path.join(projectRootDir, 'ios');
  if (fs.existsSync(iosDir)) {
    const wsOrProjectFileName = fs
      .readdirSync(iosDir)
      .find(fileName => fileName.endsWith('.xcworkspace') || fileName.endsWith('.xcodeproj'));
    if (wsOrProjectFileName) {
      return wsOrProjectFileName.split('.')[0];
    }
  }
